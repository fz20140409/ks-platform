...
var SDKBridge = function(ctr, data) {
	...
	window.nim = ctr.nim = this.nim = new SDK.NIM({
		...
		appKey: "be7c0639c10e6a69f86ce3b4fa8dc8ec",
		account: userUID,
		token: sdktoken,
	...
	function onTeams(teams) {
		...
		function getUserSession(obj,i,type,id) {
			...
			var obj1 = {
				"id": "team-" + teamlist[i].teamId,
				"to": teamlist[i].teamId,
				"scene": "team",
				"lastMsg": {
					"scene": "team",
					"target": teamlist[i].teamId,
				},
				"updateTime": teamlist[i].updateTime,
				"typeA": type
			}
			...
			function cb(err, msgObj) {
				if(msgObj && msgObj.msgs.length > 0) {
					...
					_this.cache.setSessions(_this.nim.mergeSessions(old, obj1));
					_this.controller.buildSessions(teamlist[i].teamId);
					getTalkerInfo(i + 1)
				}
			}
			_this.nim.getHistoryMsgs(param);
		}
		function getTalkerInfo(i) {
			if(i < teamlist.length) {
				var id = teamlist[i].name
				var re3 = /^(.*)(&.*)$/
