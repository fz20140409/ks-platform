...
YX.fn.openChatBox = function(account, scene) {
	...
	if(scene == "p2p") {
		...
		var name = info.name
		var re = /^(kspt_)(\w+)(#.*)$/g
		var id = name.replace(re, "$2")
		var headImg = $(".personInfo .headImg")
		var head = $("#chatTitle #headImg")
		var nickName = $("#nickName")
		var owner=info.owner
		head.attr("src", info.avatar + "?imageView&thumbnail=80x80&quality=85")
		head.attr("owner", owner)
		headImg.attr("src", info.avatar + "?imageView&thumbnail=80x80&quality=85")
		nickName.html(name)
	}
	this.doPoint()
	// 根据或取聊天记录
