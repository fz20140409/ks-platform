				...
				function showTypeList(type) {
					...
					if(sessions.length === 0) {
						...
						for(i = 0; i < sessions.length; i++) {
							...
							if(sessions[i].isNew) {
								if(localStorage.getItem(sessions[i].id)) {
									info.avatar = JSON.parse(localStorage.getItem(sessions[i].id)).avatar || "images/normal.png"
									info.nick = JSON.parse(localStorage.getItem(sessions[i].id)).name || ""
								}
							}
//							if(sessions[i].lastMsg.aaa) {
//								infoText = sessions[i].lastMsg.aaa
//							}
